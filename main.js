...
window.loadDebts = async function() {
    ...
    tbody.innerHTML = `<tr><td colspan="4" class="px-6 py-16 text-center italic text-white/20 font-sans uppercase text-[10px] tracking-widest">Calculando balance real...</td></tr>`;
    ...
    try {
        ...
        const debtList = clients.map((c, i) => ({
            clientId: c.client_id,
            name: c.name,
            totalDebt: summaries[i].deudaNeta, // Aquí ya viene restado el saldo a favor
            lastSaleDate: new Date().toISOString() // Opcional: traer fecha real
        })).filter(d => Math.abs(d.totalDebt) > 0.01); // Mostramos los que deben O tienen saldo a favor
        ...
        const renderTable = (list) => {
                    ...
                    <tr class="group border-b border-white/5 hover:bg-white/[0.02] transition-all duration-300">
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-4">
                                <div class="bg-orange-500 w-8 h-8 rounded-lg flex items-center justify-center text-white">
                        ...
                        <td class="px-10 py-6 text-right">
                            <button onclick="window.handleViewClientDebt(${debt.clientId})" 
                                class="px-6 py-2.5 rounded-lg !bg-orange-500/10 !border !border-orange-500/30 text-orange-500 hover:!bg-orange-500 hover:!text-white transition-all text-[11px] font-black uppercase tracking-widest">
                                Detalles
                            </button>
                        </td>
                    </tr>
            ...
            if (elCount) elCount.innerText = clientesConDeudaCount;
        };
        ...
        if (searchInput) {
            searchInput.oninput = (e) => {
                ...
                renderTable(debtList.filter(d => d.name.toLowerCase().includes(term)));
            };
        }

    } catch (e) {
        console.error(e);
        tbody.innerHTML = '<tr><td class="text-red-500">Error</td></tr>';
    }
};
...
window.generarPDFVenta = function() {
    ...
    const htmlContent = `
        ...
        <head>
            <meta charset="UTF-8">
            <title>Nota de Venta - ${venta.venta_id}</title>
            <style>
                @page { size: letter; margin: 15mm; }
                body { font-family: 'Segoe UI', Arial, sans-serif; color: #333; margin: 0; padding: 20px; background-color: #f4f4f4; }
                ...
                .sheet { background: white; width: 210mm; min-height: 250mm; margin: 0 auto; padding: 15mm; box-shadow: 0 0 15px rgba(0,0,0,0.1); border-radius: 4px; border-top: 12px solid ${colorOxido}; box-sizing: border-box; }
                ...
                .data-box .label { font-size: 9px; text-transform: uppercase; font-weight: bold; opacity: 0.8; margin-bottom: 5px; display: block; letter-spacing: 1px; }
                .data-box .value { font-size: 16px; font-weight: bold; letter-spacing: 0.5px; text-transform: uppercase; }
        ...
        <body>
            ...
            <div class="sheet">
                <div class="header">
                    ...
                    <div class="receipt-meta">
                        <h1>Nota de Venta</h1>
                        <div class="folio">#${venta.venta_id}</div>
                        <div class="fecha">${fecha}</div>
                    </div>
                </div>
...
window.handleDeleteClientClick = function(clientId, clientName) {
    console.log("Preparando para eliminar ID:", clientId, "Nombre:", clientName);

    // 1. Validamos que el ID exista
    if (!clientId || clientId === 'null' || clientId === 'undefined') {
        console.error("Error: ID de cliente inválido.");
        return;
    }

    // 2. Guardamos el ID en la variable global que usará el botón de confirmar
    window.clientIdToDelete = clientId;
    
    // 3. Ponemos el nombre en el modal
...
window.loadMonthlySalesReport = function(selectedMonthFromEvent, selectedYearFromEvent) {
    (async () => {
        ...
        try {
                ...
                filteredSales.forEach(sale => {
                    ...
                    finalHTML += `
                        <tr class="group hover:bg-white/[0.02] transition-all border-b border-white/5">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div class="h-9 w-9 rounded-lg bg-orange-500 flex items-center justify-center text-white shadow-lg shadow-orange-500/20 group-hover:scale-110 transition-transform duration-300">
                            ...
                            <td class="px-8 py-6 text-right">
                                ...
                                <div class="glass-badge ${hasDebt ? 'glass-badge-danger' : 'glass-badge-success'} inline-flex px-3 py-1.5 rounded-lg font-black text-lg tracking-tight">
                                    ...
                                    <span class="font-sans text-white">${window.formatCurrency(sale.saldo_pendiente)}</span>
                                </div>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end items-center gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                    
                                    <button onclick="handleViewAction(this, '${sale.venta_id}', '${sale.client_id}')" 
                                        class="group/btn h-9 w-9 flex items-center justify-center !bg-orange-500/10 !border !border-orange-500/30 rounded-lg backdrop-blur-md transition-all hover:!bg-orange-500 hover:shadow-[0_0_15px_rgba(249,115,22,0.4)]"
                                        title="Ver Detalles del Folio">
                                        <i class="fas fa-file-invoice-dollar text-orange-500 group-hover/btn:!text-white transition-colors text-lg !bg-transparent !p-0 !border-none"></i>
                                    </button>

                                    <button onclick="handleDeleteAction(this, '${sale.venta_id}', ${selectedMonth}, ${selectedYear})" 
                                        class="group/btn h-9 w-9 flex items-center justify-center !bg-red-500/10 !border !border-red-500/30 rounded-lg backdrop-blur-md transition-all hover:!bg-red-500 hover:shadow-[0_0_15px_rgba(239,68,68,0.4)]"
                                        title="Anular Venta">
                                        <i class="fas fa-trash-alt text-red-500 group-hover/btn:!text-white transition-colors text-lg !bg-transparent !p-0 !border-none"></i>
                                    </button>
            ...
            if (reportBody) reportBody.innerHTML = '<tr><td colspan="5" class="py-20 text-center text-red-500 font-black uppercase text-[10px] tracking-widest">Error de sincronización</td></tr>';
        }
    })();
};
...
// ==========================================================
// FUNCIÓN DE ELIMINACIÓN TOTAL (SIN ERRORES DE VISTA)
// ==========================================================
const inicializarEliminacionCliente = () => {
    const btnConfirmar = document.getElementById('confirm-delete-client-btn');
    if (!btnConfirmar) return;

    // Clonamos el botón para limpiar cualquier evento duplicado que cause el error de ID: null
    ...
    btnNuevo.addEventListener('click', async (e) => {
        ...
        // Rescatamos el ID guardado
        const clientId = window.clientIdToDelete;

        if (!clientId || clientId === 'null') {
            console.warn("El ID ya se procesó o es nulo.");
            return;
        }
        ...
        try {
            console.log("--- INICIANDO BORRADO EN TABLAS DE ORIGEN (NO EN LA VISTA) ---");

            // 1. Obtener IDs de ventas para borrar detalles primero
            ...
            // 2. Borrar detalles de productos (Evita error de llave foránea)
            if (ventaIds.length > 0) {
                await supabase.from('detalle_ventas').delete().in('venta_id', ventaIds);
            }

            // 3. Borrar de las tablas que alimentan tu vista 'transacciones_deuda'
            // No podemos borrar de la vista, así que borramos de las tablas reales:
            await supabase.from('pagos').delete().eq('client_id', clientId);
            await supabase.from('abonos').delete().eq('client_id', clientId);

            // 4. Borrar las ventas principales
            if (ventaIds.length > 0) {
                await supabase.from('ventas').delete().in('venta_id', ventaIds);
            }

            // 5. Borrar finalmente al cliente
            ...
            // EXITO
            showToast("Cliente y todo su historial eliminados con éxito", "success");
            closeModal('delete-client-modal');
            ...
            console.error('Falla en la cascada de borrado:', err);
...
window.showToast = function(mensaje, tipo = 'success') {
    ...
    toast.className = `
        relative bg-[#042f2e] backdrop-blur-2xl border border-white/10 
